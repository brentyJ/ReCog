...
import { Zap, FileUp, Clipboard, Users, Lightbulb, Waypoints, Settings } from 'lucide-react'
...
import { TethersPage } from './components/pages/TethersPage'
...
function App() {
  ...
  const navSections = [
    ...
    },
    {
      title: 'SETTINGS',
      items: [
        { id: 'tethers', label: 'Tethers', icon: Zap, badge: null },
      ]
    }
  ]
  ...
  const pageConfig = {
    ...
    'tethers': { title: 'Tether Management', icon: Zap },
  }
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <Scanlines />
      {/* Sidebar */}
      <aside className="w-[260px] bg-card border-r border-border flex flex-col fixed top-0 left-0 bottom-0 z-50 terminal-corners">
        {/* Logo */}
      ...
      <main className="flex-1 ml-[260px] flex flex-col">
        {/* Header */}
        <header className="h-14 bg-card border-b border-orange-mid/20 flex items-center justify-between px-6 sticky top-0 z-40">
          <h1 className="text-lg font-semibold flex items-center gap-2">
            <PageIcon className="w-5 h-5 text-orange-light" />
            <span className="text-glow">{currentPage.title}</span>
          </h1>
        ...
        <div className="flex-1 p-8 overflow-y-auto scrollbar-thin">
          ...
          {activePage === 'tethers' && <TethersPage />}
        </div>
      </main>
    </div>
  )
